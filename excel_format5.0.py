...
def process():
  ...
  for r in range(0, rows):
    ...
    if str(sheet_Report.row_values(r)[0]) == "Issue Details":
      ...
      if path_point[1] not in vuln_data['point']:
        #5.0 update:缺陷点去重
        vuln_data['point'].append(path_point[1])
        vuln_data['path'].append(path_point[0])
        no += 1
  print('[info]transfer successful,path of new excel report:')
  #输出xlsx将导致文件损坏
  outfile_name = workdir + 'Fortify Vuln Path_'+time.strftime("%Y%m%d%H%M%S",time.localtime())+'.xls'
  print('[info]'+outfile_name)
  print('[info]vuln point statistics:')
